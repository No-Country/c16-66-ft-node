...
export function AsideComponent() {
	...
	<>
	{
		...
		className={`absolute z-50 flex flex-col justify-evenly p-2 gap-2 ${
			openClose
			? "w-38 h-screen asideBackground"
			: "w-16 h-16"
			}`}
		>
			<button className='w-fit h-fit mt-2 flex justify-between' onClick={handleAsideOpenClose}>
				{openClose ? (
			...
			>
				<figure className={`w-10 h-auto ml-3 rounded-full overflow-hidden ${openClose}`}>
					{doctorLogged && (
						<Avatar
		...
		>
			<Button className='flex justify-end w-full py-1 mt-2' onClick={handleAsideOpenClose}>
				{openClose ? (
